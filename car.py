...
Max_Power = st.slider('Max Power(bhp)',15.0, 510.0, 0.1)
...
if st.button("Predict"):
        ...
        user_input = pd.DataFrame({
            ...
            'Max Power(bhp)':np.cbrt([Max_Power]),
            'Seats':[Seats],
            'Car_Age':[Car_Age]
        })
        st.write(user_input)
        ...
        final_user_df = pd.concat([user_input[['Kms_Driven','Owner_No','Engine_Displacement(cc)','Mileage(kmpl)','Max Power(bhp)','Seats','Car_Age']], encoded_user_df], axis=1)
        final_user_df

        prediction = load_model.predict(final_user_df)
